...
async def vote(ctx, question: str, duration: str, *options):
  ...
  try:
    if 'm' in duration:
      ...
      await ctx.send("時間の指定が間違っています！。例: `1m30s` もしくは `90s`")
      return
  except ValueError:
    await ctx.send("時間の形式が間違っています。例: `1m30s` もしくは `90s`")
    return

  # 選択肢の数チェック
  if len(options) < 2:
    await ctx.send("少なくとも2つの選択肢が必要ですよ！")
    return
  elif len(options) > 10:
  ...
  # 投票メッセージの作成
  description = [f"投票時間は{total_seconds // 60}分{total_seconds % 60}秒ですよ！。"]
  ...
  reaction_counts = [
      # 自分のリアクションを除外
      reaction.count - 1 for reaction in vote_message.reactions[:len(options)]]
